            ...
            <div
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; padding: 20px; background: #1e293b; color: white;">
                    ...
                    <button type="button" onclick="closePatrolScanner()"
                        ...
                        onmouseover="this.style.background='rgba(255,255,255,0.2)'" onmouseout="this.style.background='rgba(255,255,255,0.1)'">✕ Close</button>
                </div>
                ...
                <div id="patrol-qr-wrapper" style="position: relative; width: 100%; height: 450px; background: #000; overflow: hidden !important; border-radius: 8px;">
                    <style>
                        #patrol-qr-reader { width: 100% !important; height: 100% !important; border: none !important; margin: 0 !important; padding: 0 !important; }
                        ...
                        #patrol-qr-reader img[alt="Camera based scan"] { display: none !important; }
                        #patrol-qr-reader div[style*="text-align: center"] { display: none !important; }
                        #patrol-qr-reader__dashboard { display: none !important; }
                    </style>
...
?>
            ...
            <div class="card" style="border-left: 8px solid #f59e0b; padding: 0; overflow: hidden;">
                <div style="padding: 20px 25px; background: #fffcf5; border-bottom: 1px solid #fef3c7; display: flex; align-items: center; gap: 12px;">
                    <div style="background: #f59e0b; color: white; width: 30px; height: 30px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 14px;">1</div>
                            ...
                            <tbody>
                                <?php while ($t = mysqli_fetch_assoc($tickets)): ?>
                                    <tr style="cursor: pointer;"
                                        onclick="openTicketTimeline(<?php echo htmlspecialchars(json_encode([
                'id' => $t['id'],
                'status' => $t['status'],
                ...
                'updated_at' => $t['updated_at'],
                                        ...
                                        <td>
                                            <span class="badge badge-<?php echo($t['status'] == 'Open' ? 'error' : ($t['status'] == 'Assigned' ? 'warning' : 'success')); ?>">
                                                <?php echo $t['status']; ?>
                                            </span>
                                        </td>
                                        <td><?php echo $t['assigned_to_name'] ?: '<span style="color:#9ca3af">— Unassigned —</span>'; ?></td>
        ...
        <div id="ticketTimelineModal"
            ...
            <div style="background:white; border-radius:16px; width:100%; max-width:520px; max-height:90vh; overflow-y:auto; box-shadow:0 25px 50px rgba(0,0,0,0.3);">
                <!-- Modal Header -->
                <div style="background:linear-gradient(135deg,#f59e0b,#d97706); padding:20px 25px; border-radius:16px 16px 0 0; display:flex; justify-content:space-between; align-items:center;">
                    <div>
                ...
                <div style="padding:20px 25px; border-bottom:1px solid #f1f5f9;">
                    <div style="margin-bottom:10px;">
                        <strong style="color:#374151;font-size:13px;text-transform:uppercase;letter-spacing:.5px;">Issue Description</strong>
                        <p id="tlIssue" style="margin:6px 0 0 0;color:#1f2937;font-size:15px;line-height:1.5;"></p>
                    </div>
                    <div style="display:flex;gap:20px;flex-wrap:wrap;margin-top:12px;">
                        <div><small style="color:#6b7280;">📍 Location</small><br><strong id="tlLocation" style="color:#1f2937;"></strong></div>
                    ...
                    <div id="tlPhotoWrap" style="margin-top:12px;display:none;">
                        <a id="tlPhoto" href="#" target="_blank" style="color:#3b82f6;font-size:13px;">📷 View Attached Photo</a>
                    </div>
                </div>

                <!-- Timeline -->
                <div style="padding:20px 25px;">
                    <strong style="color:#374151;font-size:13px;text-transform:uppercase;letter-spacing:.5px;">📅 History Timeline</strong>
        ...
        <div id="closeModal"
            ...
            <div style="background: white; padding: 25px; border-radius: 12px; width: 90%; max-width: 400px;">
                ...
                <form method="POST">
                    ...
                    <div style="margin-top: 20px; display: flex; gap: 10px;">
                        <button type="submit" class="btn btn-secondary" style="flex: 1; background: #4b5563; color: white;">Confirm Close</button>
                        <button type="button" onclick="document.getElementById('closeModal').style.display='none'"
                            class="btn btn-secondary" style="background: #e5e7eb; color: #374151; border: 1px solid #d1d5db;">Cancel</button>
                    </div>
                </form>
            </div>
        </div>
...
?>
    ...
    endforeach; ?>
                        ...
                        <div class="form-group">
                            <label style="font-weight: 600; display: block; margin-bottom: 5px;">Material
                                Description</label>
                            <input type="text" name="material_description" id="material_description" list="mat_descs"
                                required placeholder="Search Description" onchange="fillMaterialDetails(this.value, 'desc')"
                                style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px;">
                            <datalist id="mat_descs">
                                <?php foreach ($materials as $m): ?>
                                    <option value="<?php echo htmlspecialchars($m['material_description']); ?>">
    ...
    endforeach; ?>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="supplier" id="supplier" list="sup_names"
                                placeholder="Search Supplier Name" onchange="fillSupplierDetails(this.value, 'name')"
                                style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px;">
                            <datalist id="sup_names">
                                <?php foreach ($suppliers as $s): ?>
                                    <option value="<?php echo htmlspecialchars($s['supplier']); ?>">
...
?>
            ...
            <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 16px; padding: 30px; margin-bottom: 25px; box-shadow: 0 8px 24px rgba(102, 126, 234, 0.25); display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 20px;">
                <div style="display: flex; align-items: center; gap: 15px;">
                    ...
                    <div>
                        <h1 style="margin: 0; color: white; font-size: 28px; font-weight: 700; text-shadow: 0 2px 4px rgba(0,0,0,0.2);">Manual Registers</h1>
                        <p style="margin: 5px 0 0 0; color: rgba(255,255,255,0.9); font-size: 14px;">Select a register type to log entry</p>
                    </div>
                </div>
